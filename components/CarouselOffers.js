...
const CarouselOffers = ({offers}) => {
    ...
    const ButtonGroup = ({ next, previous, goToSlide, ...rest }) => {
        ...
        return (
          <ButtonContainer>
            <LeftArrow className={currentSlide === 0 ? 'disable' : ''} onClick={() => previous()} > <RiArrowLeftSLine size="30px"/> </LeftArrow>
            <RightArrow onClick={() => next()} ><RiArrowRightSLine size="30px"/> </RightArrow>
          </ButtonContainer>
        );
      };
    ...
    return ( 
        ...
        <CarouselWrapp>
            <Present>
            <Title weight="700" color="#393939" size="1.7rem"> Volvé a soñar con tu próximo viaje.</Title>
            <PTag line="2" weight="600" color="#393939" size="1rem">Seleccionamos estas ofertas para vos, algunas con reserva flexible y otras con precios sorprendentes. Elegi la que va mejor con tus planes y compra tu próximo paquete. </PTag>
            ...
            {offers.length > 0 ? (
              ...
              >
                  ...
                  return(
                    <Link href={`/[categoria]/[id]`}  as={`/${slugify(item.tipo_producto)}/${item.id_producto}`} key={index}>
                      <Card key={index}>
                        {console.log(item, 'item')}
                          ...
                          <CardBottom>
                              ...
                              <PriceCard> $ <Be>{item.precio}</Be>Ars</PriceCard>
